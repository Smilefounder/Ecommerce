...
(function($) {
  ...
  // NOTE: The original naming is autocomplete, but it's conflicted with jQuery UI autocomplete, so I rename it to tags_autocomplete
  $.tags_autocomplete = function(element, url, options) {
  ...
  $.fn.tags_autocomplete = function(url, options) {

    return this.each(function() {
      if(undefined == $(this).data('tags_autocomplete')) {
        var plugin = new $.tags_autocomplete(this, url, options);
        $(this).data('tags_autocomplete', plugin);
      }
    });
  ...
  $.tagbox = function(element, options) {
    ...
    plugin.init = function() {
      ...
      if(plugin.settings.url) {
        ...
        plugin.input.tags_autocomplete(plugin.settings.url, $.extend({}, autocompleteDefaults, plugin.settings.autocomplete));
        
        // store the autocomplete plugin object        
        plugin.autocomplete = plugin.input.data('tags_autocomplete');
